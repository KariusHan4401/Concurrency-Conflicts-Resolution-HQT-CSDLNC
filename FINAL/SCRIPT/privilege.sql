...
AS
SELECT MATX, HO_TEN, CMND, STK, BIEN_SO, SDT, EMAIL, DUONG, PHUONG, QUAN, THANH_PHO
...
AS	
	SELECT PGH.MADH, DH.MADT, DH.TRANG_THAI, DH.PHI_VAN_CHUYEN
	FROM TAI_XE TX JOIN TAI_KHOAN TK ON TX.MATK = TK.MATK
		JOIN PHIEU_GIAO_HANG PGH ON TX.MATX = PGH.MATX
		JOIN DON_HANG DH ON DH.MADH = PGH.MADH
	WHERE TK.TENTK = CURRENT_USER --CURRENT_USER LÀ TÀI KHOẢN ĐANG ĐĂNG NHẬP 
...
-- XEM DOANH THU ĐỐI TÁC
GO
CREATE VIEW UV_DOANHTHUDT
AS
SELECT COUNT(*) AS SO_LUONG_DH, SUM(PHI_SAN_PHAM - PHI_GIAM) AS DOANH_THU
FROM DOI_TAC DT JOIN TAI_KHOAN TK ON DT.MATK = TK.MATK
	 JOIN DON_HANG DH ON DH.MADT = DT.MADT
WHERE DH.TRANG_THAI = N'Đã giao hàng' 
AND TK.TENTK = CURRENT_USER --CURRENT_USER LÀ TÀI KHOẢN ĐANG ĐĂNG NHẬP 
GRANT SELECT ON UV_DOANHTHUDT TO DOI_TAC
...
AS
SELECT DT.MADT, PP.MACN, SP.MASP, SP.TENSP, PP.GIA, PP.SO_LUONG_TON
...
-- KHÁCH HÀNG XEM CHI TIẾT ĐƠN HÀNG CỦA CHÍNH MÌNH 
CREATE VIEW UV_CTDHANG_KH 
AS
SELECT CTDH.*
FROM KHACH_HANG KH JOIN TAI_KHOAN TK ON KH.MATK = TK.MATK
JOIN DON_HANG DH ON KH.MAKH = DH.MAKH
...
GRANT SELECT ON UV_CTDHANG_KH  TO KHACH_HANG
...
--ADD MORE
GRANT SELECT ON SAN_PHAM TO KHACH_HANG, DOI_TAC, TAI_XE, NHAN_VIEN, QUAN_TRI
GRANT EXEC TO KHACH_HANG, DOI_TAC, TAI_XE, NHAN_VIEN, QUAN_TRI